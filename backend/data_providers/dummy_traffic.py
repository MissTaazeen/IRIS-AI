...
def get_traffic() -> Dict:
    cities = list(TRAFFIC_DATA.keys())
    avg_road_delay = sum(c['roads']['avg_delay_min'] for c in TRAFFIC_DATA.values() if 'roads' in c) / len([c for c in TRAFFIC_DATA.values() if 'roads' in c]) if any('roads' in c for c in TRAFFIC_DATA.values()) else 0
    return {
        'timestamp': datetime.utcnow().isoformat(),
        'cities': TRAFFIC_DATA,
        'national_avg_road_delay': avg_road_delay,
